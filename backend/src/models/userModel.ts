  ...
  username: {
    ...
    validate: {
      validator: function(username: string) {
        // Reserved usernames
        const reserved = ['admin', 'root', 'api', 'www', 'mail', 'support', 'help', 'about', 'privacy', 'terms'];
...
userSchema.pre('save', function(next) {
  if (this.isSuspended && this.suspensionExpires && this.suspensionExpires <= new Date()) {
...
userSchema.statics.searchUsers = function(query: string, limit: number = 20) {
  ...
  .select('username name bio profilePicture isVerified verificationBadge followersCount')
  .limit(limit)
