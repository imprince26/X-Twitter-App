...
export const getUserProfile = async (
  ...
  try {
    ...
    const user = await User.findOne({ username }).select(
      "-password -emailVerificationCode -passwordResetCode -twoFactorSecret -loginAttempts -lockUntil"
    );
    ...
    if (currentUserId && currentUserId !== user._id.toString()) {
      const [followStatus, followedByStatus, blockStatus, muteStatus] =
        await Promise.all([
...
export const updateUserProfile = async (
  ...
  try {
    ...
    const { name, bio, location, website, theme, language, timezone } =
      req.body;
    ...
    const updatedUser = await User.findByIdAndUpdate(userId, updateData, {
      ...
      "-password -emailVerificationCode -passwordResetToken -twoFactorSecret -loginAttempts -lockUntil"
    );
